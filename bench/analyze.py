...
def _by(results: List[Dict[str, Any]], backend: str, model: str) -> Dict[str, Dict[str, Any]]:
    out = {}
    for r in results:
...
def stage_share(results: List[Dict[str, Any]]) -> str:
    ...
    for model in models:
        ...
        s = sizes[-1]
        r = d[s]
        parts = {k: _stage(r, v) for k, v in
                 [("construct", "construct"), ("repn", "repn"),
                  ("write", "write_lp"), ("load", "load_highs")]}
        total = sum(v for v in parts.values() if v) or 1.0
        pipe = _pipeline_ms(r, "pyomo")
        load_pipe = (f"{parts['load']/pipe*100:.0f}%"
                     if (parts["load"] and pipe) else "—")
        lines.append(
...
def spike_b_tables(rep: Dict[str, Any]) -> str:
    out = ["**Coverage — which rule shapes templatize:**\n",
    ...
    out.append("| N (constraints) | vectorized == classic? | classic repn | resolve/idx | vectorized | **vec speedup** | resolve speedup |")
    out.append("|-----------------|------------------------|--------------|-------------|------------|-----------------|-----------------|")
    ...
    out.append("| N | T | rows | templatizes? | classic repn | resolve/idx | resolve speedup |")
    out.append("|---|---|------|--------------|--------------|-------------|-----------------|")
    for r in rep["sum_over_set"]["rows"]:
        rs = f"{r['resolve_s']*1000:.0f} ms" if r["resolve_s"] else "—"
        sp = f"{r['speedup_resolve_over_classic']:.2f}×" if r["speedup_resolve_over_classic"] else "—"
        out.append(
...
def main():
    ...
    ap.add_argument("--phase1", default="bench/results/phase1_network_flow.json",
    ...
    if full:
        ...
        info = full["sysinfo"]
        print("## Environment\n")
        print(f"- Pyomo `{info['packages'].get('Pyomo')}` @ commit `{(info.get('pyomo_commit') or '')[:12]}`")
        print(f"- Python {info['python']}, {info['platform']}, {info['cpu_count']} CPUs")
        pk = info["packages"]
        print(f"- numpy {pk.get('numpy')}, scipy {pk.get('scipy')}, highspy {pk.get('highspy')}, "
        ...
        for model in ["network_flow", "unit_commitment", "facility_location",
                      "facility_location_q", "supply_chain"]:
            print(f"\n### {model}\n")
