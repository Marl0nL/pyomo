...
* **Deferred (logged, not attempted):**
  - **`faststep` (warm re-solve) over a switch-ON model.** `FastStepHighs.set_instance` on a model whose constraints were *templatized* fails today (`'IndexedConstraint' has no attribute 'body'`) — a **pre-existing** Phase-3/Phase-4 gap (the templatized `compiled.rows` carry the family, not per-row bodies), reproducible with the Phase-3 switch alone and independent of this change. This change makes the columnar Var/Param side of `faststep` compatible (the objective/constraint column resolver now maps columnar `VarData`), so closing the gap is now only about exposing per-row bodies for templatized families — left as follow-up. `faststep` over a classic (switch-off) model is unaffected (42 tests green).
